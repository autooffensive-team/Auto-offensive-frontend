    ...
    >
      ...
      <div className="relative z-10 px-4 py-3 border-b border-green-500/10 grid grid-cols-3 gap-2">
        {[
          ...
          <div key={s.label} className={`bg-black/40 border ${s.border} rounded-lg p-2 text-center`}>
            <motion.div
              className={`text-xl font-(family-name:--font-fira-code) font-bold ${s.color}`}
        ...
        <div className="relative z-10">
          ...
          <div className="space-y-1.5">
            {logs.slice(-4).length > 0 ? logs.slice(-4).map((log, idx) => (
              ...
              <div className={newLocal}>awaiting output…</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="relative z-10 px-4 py-3 border-t border-green-500/15 mt-auto">
        ...
        <div className="flex items-center justify-between mb-1">
          <motion.span
            className="text-[9px] font-(family-name:--font-fira-code) text-red-500 tracking-widest uppercase"
            animate={{ opacity: [0.6, 1] }}
            transition={{ duration: 1.2, repeat: Infinity }}
          >
            Connection
          ...
          <motion.span
            className="text-[9px] font-(family-name:--font-fira-code) text-green-500 tracking-widest uppercase"
            animate={{ opacity: [0.6, 1] }}
