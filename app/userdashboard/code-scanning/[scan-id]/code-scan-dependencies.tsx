          ...
          return (
            <motion.button
              ...
              className={cn(
                ...
                active && option.color
                  ? `${option.color.bg} ${option.color.text} ring-1 ${option.color.ring}`
                  : active
...
// Dependency severity color map — matches the donut chart colors
const DEP_SEVERITY_CHIP: Record<string, { bg: string; border: string; text: string; dot: string; activeBg: string; activeText: string; activeBorder: string }> = {
  CRITICAL: { bg: "bg-red-50    dark:bg-red-950/60",    border: "border-red-300    dark:border-red-700",    text: "text-red-700    dark:text-red-300",    dot: "bg-red-500",    activeBg: "bg-red-500",    activeText: "text-white", activeBorder: "border-red-500" },
  HIGH:     { bg: "bg-orange-50 dark:bg-orange-950/60", border: "border-orange-300 dark:border-orange-700", text: "text-orange-700 dark:text-orange-300", dot: "bg-orange-500", activeBg: "bg-orange-500", activeText: "text-white", activeBorder: "border-orange-500" },
  ...
  LOW:      { bg: "bg-sky-50    dark:bg-sky-950/60",    border: "border-sky-300    dark:border-sky-700",    text: "text-sky-700    dark:text-sky-300",    dot: "bg-sky-500",    activeBg: "bg-sky-500",    activeText: "text-white", activeBorder: "border-sky-500" },
};

function DependencySeverityFilterChips({
  selected,
  onChange,
}: {
  selected: string;
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        {options.map((option) => {
          const active = selected === option.value;
          const colors = option.value ? DEP_SEVERITY_CHIP[option.value] : null;
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-500 sm:text-sm dark:text-slate-400">
          {label}
        </p>
        <p className="mt-2 text-2xl font-bold text-slate-900 sm:mt-3 sm:text-3xl md:text-4xl dark:text-white">
          {value}
        </p>
...
function DependencySeverityDistribution({
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      <h3 className="text-lg font-semibold text-slate-900 md:text-xl dark:text-white">
...
function DependencyList({
  ...
  return (
    <div className="space-y-2 sm:space-y-3">
      <div className="text-sm text-slate-500 md:text-base dark:text-slate-400">
      ...
      <div className="space-y-2 sm:space-y-3">
        {dependencies.map((dependency, idx) => (
          <motion.div
            ...
            className="rounded-lg border border-slate-200 bg-[#FCFCFA] p-3 transition-all duration-200 hover:border-teal-200 hover:bg-teal-50/40 sm:rounded-xl sm:p-4 dark:border-slate-800 dark:bg-slate-950 dark:hover:border-teal-500/20 dark:hover:bg-teal-500/5"
          >
              ...
              <div className="min-w-0 space-y-2 flex-1 sm:space-y-3">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {dependency.tool && !["npm_audit", "npm-audit"].includes(dependency.tool.toLowerCase()) && (
                     <span className="rounded-lg bg-teal-50 px-2.5 py-1 text-sm font-semibold text-teal-700 dark:bg-teal-500/10 dark:text-teal-300">
                      {formatLabel(dependency.tool)}
                    </span>
                  )}
                  {dependency.language && !["node", "nodejs"].includes(dependency.language.toLowerCase()) && (
                     <span className="rounded-lg bg-sky-50 px-2.5 py-1 text-sm text-sky-700 dark:bg-sky-500/10 dark:text-sky-300">
                      {formatLabel(dependency.language)}
                    </span>
                  )}
                  {dependency.is_vulnerable ? (
                     <span className="rounded-lg bg-red-50 px-2.5 py-1 text-sm font-semibold text-red-700 dark:bg-red-500/10 dark:text-red-300">
                      Vulnerable
                    </span>
                  ) : null}
                  {dependency.is_outdated ? (
                     <span className="rounded-lg bg-amber-50 px-2.5 py-1 text-sm font-semibold text-amber-700 dark:bg-amber-500/10 dark:text-amber-300">
                      Outdated
                    </span>
                  ) : null}
                  {dependency.has_license_issue ? (
                     <span className="rounded-lg bg-sky-50 px-2.5 py-1 text-sm font-semibold text-sky-700 dark:bg-sky-500/10 dark:text-sky-300">
                      License issue
                    </span>
                  ) : null}
                </div>
                ...
                <div>
                  ...
                  <h3 className="text-sm font-semibold text-slate-900 md:text-base dark:text-white">
                      {dependency.package_name}
                    </h3>
                    {dependency.cve_id ? (
                      <span className="rounded-lg bg-slate-100 px-2 py-0.5 text-sm font-medium text-slate-700 sm:px-2.5 sm:py-1 md:text-base dark:bg-slate-900 dark:text-slate-300">
                        {dependency.cve_id}
                      </span>
                    ) : null}
                  </div>
                  <div className="mt-2 grid gap-2 text-sm text-slate-500 sm:grid-cols-2 md:text-base xl:grid-cols-4 dark:text-slate-400">
                    <span>
                      Installed:{" "}
                ...
                {dependency.description ? (
                  <p className="line-clamp-2 text-sm leading-6 text-slate-500 md:text-base dark:text-slate-400">
                    {dependency.description}
                  </p>
                ) : null}
              </div>
          ...
          <div className="rounded-lg border border-slate-200 bg-linear-to-br from-white to-[#f7fbfb] p-3 sm:rounded-xl sm:p-4 md:p-5 dark:border-slate-800 dark:from-gray-950 dark:to-gray-950">
            ...
            <div className="space-y-4 sm:space-y-5 md:space-y-6">
              ...
              <div className="border-t border-slate-200 pt-4 sm:pt-5 md:pt-6 dark:border-slate-800">
                <DependencySeverityFilterChips
                  selected={severityFilter}
                  onChange={onSeverityFilterChange}
                />
              </div>
              <div className="border-t border-slate-200 pt-4 sm:pt-5 md:pt-6 dark:border-slate-800">
                <p className="mb-2 text-sm font-semibold text-slate-700 sm:mb-3 md:text-base dark:text-slate-300">
                  Quick flags
                </p>
                <div className="flex flex-wrap gap-2">
                  <DependencyFlag
